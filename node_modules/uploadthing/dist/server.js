...
function formatStack(stack) {
  return "  " + stack.split("\n").splice(1).map(
    (l) => l.trim().replace("file://", "").replace(process.cwd() + "/", "")
  ).join("\n  ");
}
function formatArgs(args) {
  ...
  return fmtArgs.map((arg) => {
    ...
    try {
      return (0, import_node_util.inspect)(arg, { depth: 4 });
    } catch {
...
var uploadFilesInternal = async (data, opts) => {
  ...
  if ("error" in json) {
    const error = await import_shared5.UploadThingError.fromResponse(clonedRes);
    logger.debug("Failed getting presigned URLs:", error);
    throw error;
  }
